-- “New Token Accounts” per 12h bucket, derived from token_transfers
WITH first_receive AS (
  SELECT
    LOWER(mint)              AS token_mint_address,    -- use “mint” here, all lowercase
    destination              AS account_address,       -- the wallet that received tokens
    MIN(block_timestamp)     AS first_receive_time     -- earliest time this account ever got tokens
  FROM
    `bigquery-public-data.crypto_solana_mainnet_us.token_transfers`
  WHERE
    LOWER(mint) IN (
...
SELECT
  ...
  -- Convert each first_receive_time into its 12h bucket:
  CASE
    WHEN EXTRACT(HOUR FROM first_receive_time) < 12
      THEN TIMESTAMP_TRUNC(first_receive_time, DAY) 
    ELSE TIMESTAMP_ADD(
           TIMESTAMP_TRUNC(first_receive_time, DAY),
           INTERVAL 12 HOUR
         )
  END AS bucket_start,
